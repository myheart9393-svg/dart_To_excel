...
- `scripts/investigate_*.py` 같은 조사용 스크립트는 저장소에 넣지 않는다 (.gitignore 등록됨).
- 커밋할 때 `git add -A`를 쓰지 말고 변경한 파일을 명시적으로 add한다.
...
- 재무제표 본문 HTML 은 표별로 `class="nb"` 제목표(제목·기간·단위, 1열) + `border="1"` 본문표(`<thead>` 있음) 쌍으로 구성된다. **border 가 0 아닌 값인 비 nb 표는 숫자 셀 비율과 무관하게 본문표 후보로 본다** (값이 거의 전부 `-` 인 휴면회사·손실 보고 대응, 실측 20260331000492 네오슈테른). 제목표에도 border=1 을 쓰는 문서(수양켐텍 20260331004229)가 있어, 모든 행이 1열이고 셀이 기간·단위 문구뿐인 표는 제목표로 흡수한다(statements._is_titleish_table — 기간·단위는 다음 본문표의 period_text/unit 으로).
...
- **주석 트리 제목 6계열** (parse_expected_heading, 배치 300건+실사용 실측): `1. 제목` / `14-1. 제목`(가지) / `2.1 제목`(점 가지, 휴맥스) / `19, 20. 제목`(복수 번호) / `주석1,2 - 제목`(접두, 제이앤티씨) / `주석 - 1. 제목`(접두+하이픈, 이마트 20260318001024 — 본문 구조 신호도 같은 문자열). 이들과 부산주공 합본 제목(`34. …, 35. …`)은 정규식이 아니라 **0단계(트리 자식 제목 ↔ 본문 블록 정규화 정확 일치, notes._split_by_expected)** 로 검출한다. 비정형 번호는 `Note.label`("2.1", "19, 20", "1,2")로 보존하고 시트명은 `주석02.1_…`/`주석19-20_…`/`주석01-02_…`(쉼표→하이픈, 쉼표 라벨은 각 번호 2자리). 본문 제목이 마침표로 끝나도(`11. 법인세.`) 15자 이하·공백/쉼표 없음이면 제목으로 허용.
- **총계 행 변형** (statements._find_total_rows): 정확 일치 키는 `자산총계`/`부채총계`/`자본총계` + `총자산`/`총부채`/`총자본`(신세계·INVENI) + `부채와자본총계` 류. 매칭은 **괄호 제거 전 정규화(공백만 제거) 우선, 같은 이름은 마지막 행** — `자본총계(지배기업소유주지분)` 가 진짜 `자본총계` 를 가리는 오매칭 방지(KX 실측). XBRL 표준계정명 문서(현대리바트)는 총계 행이 `자산`/`부채`/`자본`/`자본과 부채` 뿐 → 숫자 값이 있는 행(마지막 것) fallback (statements._fallback_total_rows, logging.info). 대차 불일치 경고가 남는 문서 2건(옵트론텍·태원건설)은 공시 원문 수치 자체가 안 맞는 **문서 오류** — 경고 유지가 정답.
- **재무 내용 없는 공시** (pipeline, 배치에서 E_NO_CONTENT): ① 트리에 재무 섹션이 없는 정정신고·관련 공시(related_reports 있으면 그 rcpNo 안내), ② 제출기한 연장 신고서(트리 제목에 `연장`/`제출기한`), ③ 표지 nb 표만 있고 본문표(border=1/thead)가 없는 첨부(그랜드코리아·강릉아이앤디형, `_has_no_body_table`: 본문표 0 + 텍스트 <500자; 주석도 미분류 1개 이하 또는 텍스트 <200자면 실패 처리). 모두 파서 문제가 아니라 문서 특성.
...
- **사업보고서 첨부 감사보고서**: `main.do?rcpNo=..&dcmNo=..` 로 지정된다(rcpNo 는 모공시와 같음, 실측 20260310002820: 11104486 별도·11104487 연결). main.do 에 dcmNo 를 붙이면 첨부 문서의 트리가 오고 구조는 독립 감사보고서와 동일 — 입력 URL 의 dcmNo 를 뽑아(parse_dcm_no) main.do 에 전달하면 이후 파이프라인은 무변경(노드 URL 은 트리의 자기 dcmNo 사용). 첨부 문서의 `<title>` 은 모공시 기준이라 report_name 을 "… 첨부 (연결)감사보고서" 로 보정한다.
