...
- 재무제표 본문 HTML 은 표별로 `class="nb"` 제목표(제목·기간·단위, 1열) + `border="1"` 본문표(`<thead>` 있음) 쌍으로 구성된다. **border 가 0 아닌 값인 비 nb 표는 숫자 셀 비율과 무관하게 본문표 후보로 본다** (값이 거의 전부 `-` 인 휴면회사·손실 보고 대응, 실측 20260331000492 네오슈테른). 단 제목표에도 border=1 을 쓰는 문서(수양켐텍 20260331004229)는 기간 문구 조각이 본문표로 오인될 수 있다 — 종류 판정(제목/내용)이 걸러주지 못하면 suffix 로 남는다.
