...
- **주석 트리 제목 5계열** (parse_expected_heading, 배치 300건 실측): `1. 제목` / `14-1. 제목`(가지) / `2.1 제목`(점 가지, 휴맥스) / `19, 20. 제목`(복수 번호) / `주석1,2 - 제목`(접두, 제이앤티씨). 이들과 부산주공 합본 제목(`34. …, 35. …`)은 정규식이 아니라 **0단계(트리 자식 제목 ↔ 본문 블록 정규화 정확 일치, notes._split_by_expected)** 로 검출한다. 비정형 번호는 `Note.label`("2.1", "19, 20", "1,2")로 보존하고 시트명은 `주석02.1_…`/`주석19-20_…`/`주석01-02_…`(쉼표→하이픈, 쉼표 라벨은 각 번호 2자리). 본문 제목이 마침표로 끝나도(`11. 법인세.`) 15자 이하·공백/쉼표 없음이면 제목으로 허용.
