...
- 주석 가지 번호: `14-1. 무형자산`, `14-2. 무형자산`(현대리바트), `8-1./8-2.`(부산주공)처럼 N-M 이 최상위 주석으로 쓰인다. 휴맥스형 변형: 자식 제목이 `2.1 재무제표 작성기준`, `19, 20. 영업권 및 무형자산` — 현행 정규식으로 파싱 불가(미해결).
- XBRL 표준계정명 문서(현대리바트)는 총계 행이 `자산`/`부채`/`자본`/`자본과 부채` 로 끝나고 `자산총계` 류가 없다 → 대차 검증 불가.
